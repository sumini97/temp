	...
	for (i = 0; i < NCVLOOPS; i++) {
		...
		while (testval != num) {
			...
			if (ts2.tv_sec = 0 && ts2.tv_nsec < 40 * 2000) {
				kprintf("cv_wait took onlt %u ns\n", ts2.tv_nsec);
		...
		/*
		loop a little while to make sure we can measure the time 
		waiting on the cv
		*/
...
int cvtest(int nargs, char **args) {
	...
	for (i = 0; i < NTHREADS;i++) {
		result = thread_fork("synchtest", NULL, cvtest, NULL, i);
		if (result) {
...
what we'll do is rotate through NCVS lock/CV pairs, with on thread
sleeping and the other waking it up.
If we miss a wakeup, the sleep thread won't go around enough times.
*/
